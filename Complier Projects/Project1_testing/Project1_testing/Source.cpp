...
void copy_token_string_from_buffer()
{
	...
	for (i = start_pos + 1; i <= maximal_pos; i++)
	{
		...
		//printf("(%c %c) %d %d",token[tokenLength] , (char) buffer[i], tokenLength, i);
		tokenLength++;
	}
	//printf("\n");
	token[tokenLength] = '\0';
...
int scan_id_keyword()
{
	...
	//Check INTID & REALID
	int temp_pos = next_pos;//if not these type roll back the pointer.
	if (buffer[next_pos] == '.')
	{
		next_pos++;
		int is_num = 0;
		...
		//printf("[KEN]%d %d %c\n",next_pos, temp_pos, buffer[next_pos-1]);
		//XXX.00INT is not INTID
		if (((next_pos - temp_pos > 2) && is_num) || ((next_pos - temp_pos) == 2)){
			if ((buffer[next_pos] == 'I') && (buffer[next_pos + 1] == 'N') && (buffer[next_pos + 2] == 'T')){
				ttype = INTID;
				next_pos += 3;
			}
			else if ((buffer[next_pos] == 'R') && (buffer[next_pos + 1] == 'E') &&
				(buffer[next_pos + 2] == 'A') && (buffer[next_pos + 3] == 'L')){
				ttype = REALID;
				next_pos += 4;
			}
	...
	if (ttype != INTID && ttype != REALID){
		next_pos = temp_pos;//roll back the pointer.
		maximal_pos = next_pos - 1;
		copy_token_string_from_buffer();
		ttype = isKeyword(token);//check keyword token
		if (ttype == 0)
			ttype = ID;
	}
	return ttype;
